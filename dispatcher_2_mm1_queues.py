...
SIM_TIME = 175      # time of simulation in seconds, T:{35, 175, 350, 1750}
...
class Queue(object):
    ...
    def service(self, petition, service_time):
        """The serving processes. It takes a ``petition`` process and tries
                to serve it."""
...
def petition(env, name, queue, service_time):
    ...
    with queue.server.request() as request:
        ...
        # print('%s enters the server at %.5f.' % (name, env.now))
        yield env.process(queue.service(name, service_time))

        # print('%s leaves the server at %.5f.' % (name, env.now))
...
plt.title('Equiprobable dispatcher with 2 MM1 Queues')
